package com.matt.android.mynews.models.utils;

/**
 * Util class to manage constants
 */
public class Constants {

    //Search util
    public static final String PREF_KEY_NAME = "PREF_KEY_NAME";
    public static final String PREF_KEY_SEARCH = "PREF_KEY_SEARCH";

    //Notification util
    public static final String PREF_KEY_SWITCH = "PREF_KEY_SWITCH";
    public static final String PREF_KEY_QUERY = "PREF_KEY_QUERY";
    public static final String PREF_KEY_ARTS = "PREF_KEY_ARTS";
    public static final String PREF_KEY_POLITICS = "PREF_KEY_POLITICS";
    public static final String PREF_KEY_ENTREPRENEURS = "PREF_KEY_ENTREPRENEURS";
    public static final String PREF_KEY_BUSINESS = "PREF_KEY_BUSINESS";
    public static final String PREF_KEY_SPORTS = "PREF_KEY_SPORTS";
    public static final String PREF_KEY_TRAVEL = "PREF_KEY_TRAVEL";
    public static final String PREF_KEY_NOTIFICATION_URL = "PREF_KEY_NOTIFICATION_URL";
    public static final String TAG_WORKER = "PREF_TAG_WORKER";
    public static final int ID_WORKER_NOTIFICATION = 15;
    public static final String PREF_KEY_NOTIFICATION_MESSAGE = "PREF_KEY_NOTIFICATION_MESSAGE";

}
